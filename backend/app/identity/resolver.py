...
class GitHubIdentityResolver:
    ...
    async def search(
        ...
        async with httpx.AsyncClient(
            ...
            # ---------------------------------------------------------
            # STEP 1: Try direct username resolution first.
            # If the input is actually an existing GitHub username,
            # guarantee that exact account gets priority.
            # ---------------------------------------------------------
            ...
            if exact_profile is not None:
                exact_candidate = self._build_exact_candidate(
            ...
            # ---------------------------------------------------------
            # STEP 2: Normal GitHub user search.
            # This handles human names and approximate queries.
            # ---------------------------------------------------------
            ...
            # ---------------------------------------------------------
            # STEP 3: Enrich search candidates with full public profile.
            # ---------------------------------------------------------
            ...
            # Exact username result can otherwise appear twice.
            # ---------------------------------------------------------
            deduplicated: dict[str, IdentityCandidate] = {}
            ...
            # ---------------------------------------------------------
            # STEP 5: Highest relevance first.
            # ---------------------------------------------------------
            ranked.sort(
    ...
    async def _get_profile(
        ...
        # GitHub login is limited in length, so avoid making
        # obviously invalid direct lookups unnecessarily.
        if len(username) > 39:
            return None
    ...
    def _build_candidate(
        ...
        score, reasons, match_type = self._calculate_score(
            query=query,
            username=username,
    ...
    def _calculate_score(
        ...
        # Weighted score
        #
        # Name is intentionally much more important than username
        # similarity because real usernames may be unrelated to names.
        # -------------------------------------------------------------
